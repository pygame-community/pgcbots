...
from .pgcbot import PGCBot, AutoShardedPGCBot
import pgcbots.db

from pgcbots.types import Database, Revision

PGCBotT = PGCBot | AutoShardedPGCBot
...
async def migrate(
    ...
    revision_number = await pgcbots.db.get_pgcbots_db_schema_revision_number(db)
    ...
    if final_steps is None or final_steps > 0:  # migration
        ...
        initial_migration_count = (
            ...
            db, revisions, bot_name, bot_uid, auto_migrate
        )
        ...
        if final_steps is None or final_steps >= 1:
            migration_count += await pgcbots.db.migrate_pgcbots_db_schema(
                db,
                revisions,
                final_steps,
            )
        ...
        if await pgcbots.db.pgcbots_db_schema_is_defined(db):
            rollback_count = await pgcbots.db.rollback_pgcbots_db_schema(
                db, revisions, abs(final_steps)
            )
                ...
                confirm = click.confirm(
                    click.style(
                        ...
                        + str(
                            revision_number + final_steps
                            if final_steps
                            else len(extension_manager.revisions) - 1
                        )
                        + " is about to be performed."
                        "\nDo you wish to proceed?",
                        bold=True,
...
from pgcbots.extension_core import ExtensionManager
