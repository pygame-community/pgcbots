...
This file implements a CLI with boilerplate functionality for bot
...
import pgcbots.db
from pgcbots.types import Revision
import pgcbots.utils
from pgcbots.utils import _raise_exc, ParserMapping, ParserMappingValue

from .pgcbot import PGCBot
from . import constants, cli_helpers

from .config_parsing import (
    parse_databases,
    parse_extensions,
    parse_main_database_name,
    default_config_parser_mapping,
)
...
def make_bot_cli(
    ...
    botconfig_parser_mapping: ParserMapping | None = None,
    default_extensions: list[dict[str, Any]] | None = None,
    bot_kwargs: dict[str, Any] | None = None,
):
    _default_config_parser_mapping: ParserMapping = default_config_parser_mapping | (botconfig_parser_mapping or {})  # type: ignore

    bot_name = bot_name or bot_cls.__name__
    ...
    def setup_logging(log_level, stdout: bool = True) -> None:
        ...
        if stdout:
            ...
            if discord.utils.stream_supports_colour(stream_handler.stream):
                stream_formatter = pgcbots.utils.ANSIColorFormatter()
            else:
                stream_formatter = pgcbots.utils.DefaultFormatter(
        ...
        rotating_file_handler = pgcbots.utils.RotatingFileHandler(
        ...
        rotating_file_handler.setFormatter(
            pgcbots.utils.DefaultFormatter(
                "[{asctime}] [ {levelname:<8} ] {name} -- {message}", style="{"
            )
        )
    ...
    async def run_bot(bot: PGCBot) -> None:
        try:
    ...
    async def close_bot(bot: PGCBot) -> None:
        print("\nClosing bot...")
        await bot.close()
    ...
    def load_config_files(
        ...
        if config_path:
            # load config data
            try:
                config_module = pgcbots.utils.import_module_from_path(
                    "config", config_path
                )
                try:
        ...
        if localconfig_path:
            ...
            try:
                localconfig_module = pgcbots.utils.import_module_from_path(
    ...
    def main(
        ...
        try:
            config.update(
                _default_config_parser_mapping.parse(
                    config | dict(_default_extensions=default_extensions)
                )
            )
    ...
    def migrate(
        ...
        async def _run():
            try:
                db, *_ = await pgcbots.db.load_databases([main_db_info_dict])
                await cli_helpers.migrate(
                    db,
                    bot_name,
                    bot_uid,
                    specifier,
                    revisions,
                    config.get("auto_migrate", False),
                    quiet,
                    yes,
                )
                await pgcbots.db.unload_databases([db])
            except click.Abort:
                raise
    ...
    def extensions_info(
        ...
        async def _run():
            try:
                main_db_dict = (await pgcbots.db.load_databases([main_db_info_dict]))[0]
            ...
            await pgcbots.db.unload_databases([main_db_dict])
    ...
    def extensions_delete(
        ...
        if all_extensions and local_extensions:
            click.secho(
                "Flags -a/--all and -l/--local are mutually exclusive.",
                err=True,
                fg="red",
            )
            raise click.Abort()
        ...
        try:
            config.update(
                ...
                ).parse(config | dict(_default_extensions=default_extensions))
            )
        except ParserMapping.ParsingError as p:
            click.secho(f"  config error: {p.args[0]}", err=True, fg="red")
            raise click.Abort()
    ...
    def extensions_migrate(
        ...
        if not (option or all_option or local_option):
            click.secho(
                f"Flags '--option', '--all-option' and '--local-option' cannot "
                "all be empty/omitted.",
                err=True,
                fg="red",
            )
            raise click.Abort()
            ...
            click.secho(
                "Flags -a/--all-option and -l/--local-option are mutually "
                "exclusive.",
                err=True,
                fg="red",
            )
        ...
        async def _run():
            ...
            if all_option:
                options = tuple(
                    (name, all_option)
                    for name in await pgcbots.db.get_extension_data_names(main_db_dict)
                )
    ...
    @click.option("-l", "--local", "local_extensions", is_flag=True, default=False,
                help="Only apply the specified changes to locally available bot "
                "extensions.")
    @click.option("-p", "--pragma", "pragma", type=(str, str),
                  help="A pragma variable to set.", multiple=True)
    @shared_options("config_path", "localconfig_path", "log_level", "ignore_failures", 
                    "quiet", "help")
    # fmt: on
    def extensions_set(
        ...
        async def _run():
            ...
            if all_extensions:
                extensions = await pgcbots.db.get_extension_data_names(main_db_dict)
                if not extensions:
                    click.secho(
            ...
            for extension_name in extensions:
                if not await pgcbots.db.extension_data_exists(
                    main_db_dict, extension_name
                ):
                    if not quiet:
                        click.secho(
