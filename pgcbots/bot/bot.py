...
class BotBase(discord.ext.commands.bot.BotBase):
    if TYPE_CHECKING:  # Trick type-checker to reuse constructor of superclass
    ...
    async def load_extension_with_config(
        ...
        *,
        ...
        """A shorthand for calling :meth:`set_extension_config` followed by :meth:`load_extension`."""
    ...
    async def teardown_hook(self) -> None:
        """An analogue to :meth:`setup_hook()` that is called only once, within :meth:`close()`.
...
class Bot(BotBase, commands.Bot):
    """A drop-in replacement for :class:`discord.ext.commands.Bot` with more extension-loading features."""

    pass


class AutoShardedBot(BotBase, commands.AutoShardedBot):
    """A drop-in replacement for :class:`discord.ext.commands.AutoShardedBot` with more extension-loading features."""
