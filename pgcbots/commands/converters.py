...
import pgcbots
from pgcbots.utils import regex_patterns
...
class DateTimeConverter(commands.Converter[datetime.datetime]):
    ...
    async def convert(
        ...
        if pgcbots.utils.is_markdown_timestamp(arg):
            arg = pgcbots.utils.extract_markdown_timestamp(arg)
            try:
...
class CodeBlock:
    ...
    `pgcbots.commands.converters.FlagConverter` instead.
    """
...
class ParensConverter(commands.Converter[tuple]):
    ...
    is recommended to subclass `pgcbots.commands.converters.FlagConverter`
    instead.
...
class UnicodeEmojiConverter(commands.Converter[str]):
    ...
    async def convert(self, ctx: commands.Context[_BotT], argument: str) -> str:
        ...
        if pgcbots.utils.is_emoji_shortcode(argument):
            return pgcbots.utils.shortcode_to_unicode_emoji(argument)

        elif pgcbots.utils.is_unicode_emoji(argument):
            return argument
